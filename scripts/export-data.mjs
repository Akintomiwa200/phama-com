...
// Generate full patient data matching the database.ts structure
const patients = [
  {
...
// Write expanded JSON files
writeFileSync(resolve(dataDir, "patients.json"), JSON.stringify(patients, null, 2));
writeFileSync(resolve(dataDir, "interactions.json"), JSON.stringify(interactions, null, 2));
writeFileSync(resolve(dataDir, "cascade-patterns.json"), JSON.stringify(cascadePatterns, null, 2));
writeFileSync(resolve(dataDir, "pharmacists.json"), JSON.stringify(pharmacists, null, 2));
writeFileSync(resolve(dataDir, "prescriptions.json"), JSON.stringify(prescriptions, null, 2));
writeFileSync(resolve(dataDir, "inventory.json"), JSON.stringify(inventory, null, 2));
writeFileSync(resolve(dataDir, "audit-log.json"), JSON.stringify(auditLog, null, 2));

console.log("Exported expanded data files");
console.log(`  patients.json: ${patients.length} patients`);
console.log(`  interactions.json: ${interactions.length} interactions`);
console.log(`  cascade-patterns.json: ${cascadePatterns.length} patterns`);
console.log(`  pharmacists.json: ${pharmacists.length} pharmacists`);
console.log(`  prescriptions.json: ${prescriptions.length} prescriptions`);
console.log(`  inventory.json: ${inventory.length} items`);
console.log(`  audit-log.json: ${auditLog.entries.length} entries`);
